...
export const VisualizationPageName: React.FC<VisualizationPageProps> = () => {
  ...
  }, [neededNameSimilarity, notNeededNameSimilarity, dimensions]);
  ...
  const createBarGraph = () => {
    ...
    const data = [
      { category: 'Needed', value: neededNameSimilarity },
      { category: 'Not Needed', value: notNeededNameSimilarity }
    ];
    ...
    g.selectAll('.bar-label')
      ...
      .text(d => `${d.value} (${((d.value / (neededNameSimilarity + notNeededNameSimilarity)) * 100).toFixed(1)}%)`);
  ...
  const createPieChart = () => {
    ...
    const color = d3.scaleOrdinal()
      .domain(['Needed Name Similarity', 'Not Needed'])
      .range(['#E0E1A7', '#A8DEC6']);
    ...
    arcs.append('text')
      ...
      .attr('text-anchor', 'middle')
      .text(d => `${d.data.category}: ${d.data.value} (${((d.data.value / (neededNameSimilarity + notNeededNameSimilarity)) * 100).toFixed(1)}%)`);
  };
    ...
    <div>
      <div style={{
        ...
        <h1 style={{
          ...
          Name Similarity Analysis in HERE API Results
        </h1>
      </div>
      <div style={{ display: 'flex', justifyContent: 'space-around', flexWrap: 'wrap' }}>
        <div style={{ width: '100%', maxWidth: '1000px', margin: '20px auto' }}>
          <h2 style={{ textAlign: 'center' }}>Bar Graph: Name Similarity</h2>
          <svg ref={barChartRef} style={{ width: '100%', height: 'auto' }}></svg>
        </div>
        <div style={{ width: '100%', maxWidth: '1000px', margin: '20px auto' }}>
          <h2 style={{ textAlign: 'center' }}>Pie Chart: Name Similarity</h2>
